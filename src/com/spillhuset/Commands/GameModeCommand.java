        ...
        if (command.getName().equalsIgnoreCase("gamemode")) {
            ...
            if (strings.length == 2) {
                ...
                if (target == null || !target.isOnline()) {
                    OddJob.getInstance().getMessageManager().errorPlayer(Plugin.gamemode,strings[1],commandSender);
                    return true;
                }
            ...
            if (target != null) {
                //TODO OddJob.getInstance().getPlayerManager().setGameMode(target, gm);
                OddJob.getInstance().getMessageManager().gamemmodeChanged(target.getName(),commandSender);
            }
        }
        return true;
    }
