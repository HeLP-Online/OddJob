...
public class PlayerBlacklistRemoveCommand extends SubCommand {
    ...
    public void perform(CommandSender sender, String[] args) {
        ...
        if (!oddPlayer.getBlacklist().contains(target)) {
            OddJob.getInstance().getMessageManager().blacklistNotFound(args[2],sender);
            return;
        }

        oddPlayer.removeBlacklist(target);
        OddJob.getInstance().getMessageManager().blacklistDel(args[2], sender);
    }
    ...
    public List<String> getTab(CommandSender sender, String[] args) {
        ...
        OddPlayer oddPlayer = OddJob.getInstance().getPlayerManager().getOddPlayer(((Player)sender).getUniqueId());
        if (oddPlayer.getBlacklist().size() > 0) {
