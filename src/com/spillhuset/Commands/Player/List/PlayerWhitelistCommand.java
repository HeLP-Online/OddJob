...
public class PlayerWhitelistCommand extends SubCommand {
    private final List<SubCommand> subCommands = new ArrayList<>();

    public PlayerWhitelistCommand() {
        subCommands.add(new PlayerWhitelistAddCommand());
        subCommands.add(new PlayerWhitelistRemoveCommand());
    }
    ...
    public void perform(CommandSender sender, String[] args) {
        ...
        if (args.length == 1 && can(sender,false)) {
            ...
            OddJob.getInstance().getMessageManager().playerWhitelist(target,sender);
        } else {
